...
MPU6050_STATUS verify;
MPU6050_VALUES readValues;
MOVIMENT dir;
...
void MouseInterpreter(void){
	// Byte 0 - CLICK do mouse B0 >> Esquerdo | B1 >> meio | B2 >> Direito
	// Byte 1 - POSIÇÃO X
	// Byte 2 - POSIÇÃO Y
	// Byte 3 - SCROLL

	buf_mouse[0] = (!HAL_GPIO_ReadPin(MOUSE_CLICK_GPIO_Port, MOUSE_CLICK_Pin));

	switch(dir){
		case FRONT: buf_mouse[2] = MOUSE_SPD; break;
		case BACK: buf_mouse[2] = -MOUSE_SPD; break;
		case LEFT: buf_mouse[1] = -MOUSE_SPD; break;
		case RIGHT: buf_mouse[1] = MOUSE_SPD; break;
		case STOPED:{
			buf_mouse[1] = 0;
			buf_mouse[2] = 0;
		}
...
//	  USBD_HID_SendReport(&hUsbDeviceFS, buf_mouse, 4);
